...
const executeConfigTransaction = {
  ...
  async run(args: {
    ...
    try {
      ...
      if (!tx || !tx.data) {
        return mcpError("Failed to send config transaction execution");
      }
      return mcpText(
        JSON.stringify(
          {
      ...
      return mcpError("Failed to execute config transaction", e?.message);
    }
  },
};

export default executeConfigTransaction;
